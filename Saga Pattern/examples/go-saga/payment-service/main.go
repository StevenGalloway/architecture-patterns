...
const qPayCmd = "payment.commands"
...
func main() {
	...
	msgs, err := ch.Consume(qPayCmd, "", false, false, false, false, nil)
	if err != nil { panic(err) }

	logf("payment", "listening on %s", qPayCmd)
	...
	for d := range msgs {
		...
		case "AuthorizePayment":
			...
			publishEvent(ch, "PaymentAuthorized", e.SagaID, map[string]any{"order_id": o.OrderID, "amount": o.Amount})
			logf("payment", "authorized saga=%s order=%s", e.SagaID, o.OrderID)

		case "RefundPayment":
			time.Sleep(100 * time.Millisecond)
			publishEvent(ch, "PaymentRefunded", e.SagaID, map[string]any{"order_id": o.OrderID, "amount": o.Amount})
			logf("payment", "refunded saga=%s order=%s", e.SagaID, o.OrderID)
		}
